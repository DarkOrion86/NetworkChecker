    ...
    public class DeviceForPing : IDevice
    {
        ...
        public DeviceForPing(string config)
        {
            ...
            try
            {
                ...
                Program.logger.Debug("Adding device Name=" + Name + ", IP = "
                                + DeviceIP + ", timeinterval = " + TimeInterval);
            }
            catch (Exception e)
            {
                Program.logger.Error($"Faulted while creating DeviceForPing: {config}" + e.Message);
            }
        }
        ...
        public async Task<string[]> CheckDevice()
        {
            try
            {
                ...
                Program.logger.Debug($"{this.Name}:Ping {this.DeviceIP} {replyStatus}, Latency = {(replyTime).ToString()}");
                return new string[] { (replyTime).ToString() };
            }
            catch (Exception e)
            {
                Program.logger.Error($"Faulted while checking DeviceForPing: {this.Name}, {this.DeviceIP}, {this.TimeInterval}" + e.Message);
                return null;
            }
        }
        ...
        public void PushDataToPrometheus(string[] args)
        {
            ...
            {
                Program.logger.Error("Faulting to push data: " + e.Message);
            }
        }
        ...
        public bool IsParametersCorrect(string config)
        {
            ...
            {
                Program.logger.Error("Incorrect parameters: " + e.Message);
                return false;
            }
        }
